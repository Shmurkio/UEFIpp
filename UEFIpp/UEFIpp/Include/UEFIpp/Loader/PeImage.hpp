...
namespace UEFIpp::PeImage
{
	...
	[[nodiscard]] inline auto IsImage64(const Foundation::Void* ImageBase) -> Foundation::Bool
	{
		...
		if (!IsDosHeaderValid(*Dos))
		{
			return false;
		}

		const auto* Nt = NtHeaders64Of(ImageBase);
		return IsNtHeaderValid(*Nt);
	}

	[[nodiscard]] inline auto ImageSize64(const Foundation::Void* ImageBase)
		-> Foundation::Uint32
	{
		return NtHeaders64Of(ImageBase)->OptionalHeader.SizeOfImage;
	}
}