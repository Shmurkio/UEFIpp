...
namespace Cast
{
    namespace Detail
    {
        ...
        template<typename T>
        struct IsPointer<const T* const> : TrueType
        {
        };

        template<typename T>
        struct IsPointer<const T* volatile> : TrueType
        {
        };

        template<typename T>
        struct IsPointer<const T* const volatile> : TrueType
        {
        };
        ...
        template<typename T>
        struct IsPointer<volatile T* const> : TrueType
        {
        };

        template<typename T>
        struct IsPointer<volatile T* volatile> : TrueType
        {
        };

        template<typename T>
        struct IsPointer<volatile T* const volatile> : TrueType
        {
        };
        ...
        template<typename T>
        struct IsPointer<const volatile T* const> : TrueType
        {
        };

        template<typename T>
        struct IsPointer<const volatile T* volatile> : TrueType
        {
        };

        template<typename T>
        struct IsPointer<const volatile T* const volatile> : TrueType
        {
        };
    ...
    constexpr auto To(TFrom&& Value) -> decltype(auto)
    {
        ...
        else if constexpr (Detail::IsPointer<TTo>::Value && Detail::IsPointer<TFrom&&>::Value)
        {
            using ToPointee = typename Detail::RemovePointer<typename Detail::RemoveCvRef<TTo>::Type>::Type;
            using FromPointee = typename Detail::RemovePointer<typename Detail::RemoveCvRef<TFrom>::Type>::Type;
            static_assert(!Detail::IsConst<FromPointee>::Value || Detail::IsConst<ToPointee>::Value, "Cast::To<TTo>(Value): cannot cast away const from pointer");
            return reinterpret_cast<TTo>(Value);
        }
