...
auto Environment::LoadFile(const String& FilePath, FILE& File) -> bool
{
	File = {};
	...
	for (uint64_t i = 0; i < FsCount; ++i)
	{
		...
		if (!OpenFileFromFs(FsHandles[i], FilePath, Buffer, Size))
		{
			continue;
		}
...
auto Environment::UnloadFile(FILE& File) -> bool
{
	auto Success = true;

	if (File.FileBuffer)
	{
		Success = Memory::FreePool(File.FileBuffer, true);
	}
	else
	{
