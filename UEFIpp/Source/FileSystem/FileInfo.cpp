...
auto FileInfo::Path() const -> const FileSystem::Path & { return Path_; }

auto FileInfo::Name() const -> Library::StringView { return Path_.FileName(); }
...
auto FileInfo::Stem() const -> Library::StringView { return Path_.Stem(); }

auto FileInfo::HasExtension() const -> Foundation::Bool {
  return Path_.HasExtension();
}

auto FileInfo::Size() const -> Foundation::Uint64 { return Size_; }

auto FileInfo::PhysicalSize() const -> Foundation::Uint64 {
  return PhysicalSize_;
}

auto FileInfo::Empty() const -> Foundation::Bool { return Size_ == 0; }
...
auto FileInfo::IsFile() const -> Foundation::Bool { return !IsDirectory(); }
...
FileInfo::FileInfo(const FileSystem::Path &Path, Foundation::Uint64 Size,
                   Foundation::Uint64 PhysicalSize,
                   const FileSystem::Time &CreatedTime,
                   const FileSystem::Time &ModifiedTime,
                   const FileSystem::Time &AccessedTime,
                   const FileSystem::FileAttributes &Attributes,
                   Memory::AllocatorStub Allocator)
    : Path_(Path, Allocator), Size_(Size), PhysicalSize_(PhysicalSize),
      CreatedTime_(CreatedTime), ModifiedTime_(ModifiedTime),
      AccessedTime_(AccessedTime), Attributes_(Attributes) {}
} // namespace UEFIpp::FileSystem
