...
namespace UEFIpp::Library
{
	...
	public:
		...
		constexpr explicit BasicEvent(Memory::AllocatorStub Allocator) noexcept :
			Handlers_(Allocator)
		{
		}
		...
		{
			return Connect(Handler{ Foundation::Utility::Forward<Callable>(Callback), Handlers_.Allocator() });
		}

		template<typename Callable>
		[[nodiscard]] auto SubscribeOnce(Callable&& Callback) -> EventConnection
		{
			return ConnectOnce(Handler{ Foundation::Utility::Forward<Callable>(Callback), Handlers_.Allocator() });
		}
		...
		auto operator+=(Handler Callback) -> EventConnection
		{
