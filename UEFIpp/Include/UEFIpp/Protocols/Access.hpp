...
public:
  ...
  constexpr explicit Access(BootServicesType *BootServices,
                            Memory::AllocatorStub Allocator = {})
      : BootServices_(BootServices), Allocator_(Allocator) {}
  ...
  [[nodiscard]] auto Locate() const -> Library::Optional<TProtocol *> {
    ...
    const auto Result = BootServices_->LocateProtocol(
        Foundation::Cast::Auto<UEFI::Guid *>(&ProtocolTraits<TProtocol>::Id),
        nullptr, &Interface);
  ...
  template <typename TProtocol>
  [[nodiscard]] auto Handle(UEFI::Handle Handle) const
      -> Library::Optional<TProtocol *> {
    if (!BootServices_ || !Handle) {
      return Library::NullOpt;
    }

    Foundation::Void *Interface{};
    const auto Result = BootServices_->HandleProtocol(
        Handle,
        Foundation::Cast::Auto<UEFI::Guid *>(&ProtocolTraits<TProtocol>::Id),
        &Interface);
  ...
  template <typename TProtocol>
  [[nodiscard]] auto LocateHandles() const
      -> Library::Optional<Library::Vector<UEFI::Handle>> {
    if (!BootServices_) {
      return Library::NullOpt;
    }
    ...
    auto Result = BootServices_->LocateHandle(
        UEFI::Table::LocateSearchType::ByProtocol,
        Foundation::Cast::Auto<UEFI::Guid *>(&ProtocolTraits<TProtocol>::Id),
        nullptr, &BufferSize, nullptr);

    if (Result != UEFI::StatusCode::BufferTooSmall) {
      return Library::NullOpt;
    }

    Library::Vector<UEFI::Handle> Handles{Allocator_};
    if (!Handles.Resize(BufferSize / sizeof(UEFI::Handle))) {
      return {};
    }

    Result = BootServices_->LocateHandle(
        UEFI::Table::LocateSearchType::ByProtocol,
        Foundation::Cast::Auto<UEFI::Guid *>(&ProtocolTraits<TProtocol>::Id),
        nullptr, &BufferSize, Handles.Data());
  ...
  template <typename TProtocol>
  [[nodiscard]] auto Install(UEFI::Handle *Handle, TProtocol *Interface) const
      -> UEFI::StatusCode {
    if (!BootServices_ || !Handle || !Interface) {
      return UEFI::StatusCode::InvalidParameter;
    }

    return BootServices_->InstallProtocolInterface(
        Handle,
        Foundation::Cast::Auto<UEFI::Guid *>(&ProtocolTraits<TProtocol>::Id),
        UEFI::Table::InterfaceType::Native, Interface);
  }

  template <typename TProtocol>
  [[nodiscard]] auto Uninstall(UEFI::Handle Handle, TProtocol *Interface) const
      -> UEFI::StatusCode {
    if (!BootServices_ || !Handle || !Interface) {
      return UEFI::StatusCode::InvalidParameter;
    }

    return BootServices_->UninstallProtocolInterface(
        Handle,
        Foundation::Cast::Auto<UEFI::Guid *>(&ProtocolTraits<TProtocol>::Id),
        Interface);
  }
