...
namespace UEFIpp::Stream
{
	...
	public:
		...
		constexpr explicit Input(Memory::AllocatorStub Allocator) noexcept :
			OnKey(Allocator),
			OnCharacter(Allocator),
			OnLine(Allocator)
		{
		}

		constexpr explicit Input(
			TSource Source,
		...
		auto ReadLine(Library::String& String) -> Foundation::Void
		{
			...
			for (;;)
			{
				...
				(void)String.PushBack(Character);
			}
