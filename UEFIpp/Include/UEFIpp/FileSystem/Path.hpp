...
public:
  ...
  constexpr explicit Path(Memory::AllocatorStub Allocator) noexcept
      : Path_(Allocator) {}

  // Constructs a path from a string
  Path(const Library::String &Path, Memory::AllocatorStub Allocator = {});

  // Constructs a path from a C-style string
  Path(const Foundation::Char *Path, Memory::AllocatorStub Allocator = {});

  // Constructs a path from a string view
  Path(Library::StringView Path, Memory::AllocatorStub Allocator = {});

  // Copies another path
  Path(const Path &) = default;

  // Copies another path into a specific allocator.
  Path(const Path &Other, Memory::AllocatorStub Allocator);
  ...
  // Returns the components of the path as a vector of string views, split by
  // backslashes
  ...
  // Joins two paths together, inserting a backslash if necessary and
  // normalizing the result
  [[nodiscard]] static auto Join(const Path &Left, const Path &Right,
                                 Memory::AllocatorStub Allocator = {}) -> Path;

  // Joins this path with another path, inserting a backslash if necessary and
  // normalizing the result
  [[nodiscard]] auto Join(const Path &Other) const -> Path;

  // Returns the parent path by removing the last component, or an empty path if
  // there is no parent
  [[nodiscard]] auto Parent() const -> Path;

  // Returns the file name component of the path (the last component), or an
  // empty string view if there are no components
  [[nodiscard]] auto FileName() const -> Library::StringView;

  // Returns the file extension of the path (the substring after the last '.' in
  // the last component), or an empty string view if there is no extension
  [[nodiscard]] auto Extension() const -> Library::StringView;

  // Returns the file stem of the path (the last component without the
  // extension)
  ...
  // Returns a new path with the file extension replaced by the given extension,
  // or added if there is no existing extension. The extension should not
  // include the '.' character
  [[nodiscard]] auto ReplaceExtension(Library::StringView Extension) const
      -> Path;

  // Returns a new path with the file extension removed
  [[nodiscard]] auto RemoveExtension() const -> Path;

  // Returns a new path that is relative to the given base path, by removing the
  // common prefix components and adding ".." components as necessary
  [[nodiscard]] auto RelativeTo(const Path &Base) const -> Path;

  // Compares two paths for equality
  [[nodiscard]] auto operator==(const Path &Other) const
